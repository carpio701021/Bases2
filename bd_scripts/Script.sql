...
CREATE TABLE IF NOT EXISTS `OutGuat`.`Calificacion` (
  ...
  PRIMARY KEY (`idEstablecimiento`, `idServicio`, `idUsuario`)  COMMENT '',
...
CREATE TABLE IF NOT EXISTS `OutGuat`.`Comentario` (
  ...
  `idServicio` INT NOT NULL COMMENT '',
  `idUsuario` INT NOT NULL COMMENT '',
  `comentario` VARCHAR(128) NOT NULL COMMENT '',
  `fecha` DATETIME NOT NULL COMMENT '',
  PRIMARY KEY (`id`)  COMMENT '',
  INDEX `fk_Comentario_Calificacion1_idx` (`idServicio` ASC, `idEstablecimiento` ASC, `idUsuario` ASC)  COMMENT '',
  CONSTRAINT `fk_Comentario_Calificacion1`
    FOREIGN KEY (`idServicio` , `idEstablecimiento` , `idUsuario`)
    REFERENCES `OutGuat`.`Calificacion` (`idServicio` , `idEstablecimiento` , `idUsuario`)
    ON DELETE CASCADE
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
